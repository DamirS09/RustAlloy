use alloy::{network::TransactionBuilder,
            primitives::{address,utils::{format_ether, Unit}, U256},
...
async fn main() -> Result<(), Box<dyn Error>> {
    let signer: PrivateKeySigner = "0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80".parse()?; //connect wallet with private key
    let provider = ProviderBuilder::new().wallet(signer).connect("http://127.0.0.1:8545").await?; // connect to blockchain with wallet
    ...
    let tx = TransactionRequest::default().with_to(sabina).with_value(value); //prepare tx
